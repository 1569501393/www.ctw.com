	...
	<form action="<?php echo u('public/login');?>" method="post" id="myform">
		...
		<dl>
			...
			<?php if($set["check_code"] == '1'): ?><dd>验　证　码：<input id="verify" type="text" name="verify">
					<img id="verify_img" src="<?php echo U('verify/verify',array('rand'=>time()));?>" title="看不清？点击刷新" onclick="javascript:this.src='<?php echo U('verify/verify/');?>&mt='+Math.random()" />
				</dd><?php endif; ?>
