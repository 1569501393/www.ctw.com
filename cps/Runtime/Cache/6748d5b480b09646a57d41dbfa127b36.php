...
<div id="header">
    ...
    <div class="col-auto" style="overflow: visible">
        <div class="log white cut_line"> <?php echo (L("hello")); ?>！<?php echo ($my_info["user_name"]); ?> [<?php echo ($admin_level["name"]); ?>]
              | 
              <a href="javascript:edit(<?php echo ($_SESSION['admin_info']['id']); ?>,'<?php echo ($_SESSION['admin_info']['user_name']); ?>')">[修改密码]</a>   |  
...
function edit(id, name) {
	var lang_edit = "<?php echo (L("edit")); ?>";
	window.top.art.dialog({id:'edit'}).close();
	window.top.art.dialog({title:lang_edit+'--'+name,id:'edit',iframe:'?m=admin&a=pwd&id='+id,width:'480',height:'450'}, function(){var d = window.top.art.dialog({id:'edit'}).data.iframe;d.document.getElementById('dosubmit').click();return false;}, function(){window.top.art.dialog({id:'edit'}).close()});
}
