...
//		array('cate_id', '商品分类'),
        ...
        $xlsData = $xlsModel->Field('id,item_id,title,cate_id,cate_name,price,rate,commission,add_time')->where('con_id=' . $_GET['id'])->order('id DESC')->select();
...
//        $objPHPExcel->getActiveSheet(0)->mergeCells('A1:' . $cellName[$cellNum - 1] . '1');//合并单元格  屏蔽
...
//        $objPHPExcel->getactivesheet()->mergeCells('A1:' . $cellName[$cellNum2 - 1] . '1');//合并单元格  屏蔽
        // $objPHPExcel->setActiveSheetIndex(0)->setCellValue('A1', $expTitle.'  Export time:'.date('Y-m-d H:i:s'));
        for ($i = 0; $i < $cellNum2; $i++) {
            $objPHPExcel->getactivesheet()->setCellValue($cellName[$i] . '1', $header['cell'][$i][1]);
        }
        // Miscellaneous glyphs, UTF-8
        for ($i = 0; $i < $dataNum2; $i++) {
            for ($j = 0; $j < $cellNum2; $j++) {
                $objPHPExcel->getactivesheet()->setCellValue($cellName[$j] . ($i + 2), $header['data'][$i][$header['cell'][$j][0]]);
            }
        }
...
//				var_dump($_POST);exit;
                ...
                if (($_POST['rate'] && $_POST['commission']) && ((string)$_POST['rate'] != ($_POST['commission'] / $_POST['price'] * 100))) {
                    $this->error('佣金和佣金比例设置不对，请重新设置！');
                }
