...
class testAction extends baseAction {
	function index() {
	    ...
	    // $mobile = M('users')->where(array('user_id'=> $user_id))->getField('mobile_phone');

        // 适应wap端抽奖
        if (!$user_id){
//            $user_id = M('users')->where(array('mobile_phone'=> $_REQUEST['mobile_phone']))->getField('user_id');
            $user_id = M('user')->where(array('mobile'=> $_REQUEST['mobile_phone']))->getField('id');
        }

        dump(M('user')->getLastSql()); // string(72) "SELECT `id` FROM `ctw_user` WHERE ( `mobile` = '18959269002' ) LIMIT 1  "
        dump($user_id);exit;



	    $adminList = M('admin')->select();
        var_dump(C('INDEX_GROUP_CATES'));
        var_dump(S('index_group_cates'));
        dump($adminList);exit;
        //分类数据
		$focus_mod = D('focus');
