...
<link rel="stylesheet" type="text/css" href="__TMPL__public/css/album_index.css" />   
...
<div class="content">
	<div id="group_nav" class="clearfix">
		<ul>
			<li><a href="<?php echo u('album/index');?>" <?php if($cid == -1): ?>class="on"<?php endif; ?>>推荐</a></li>
			<?php if(is_array($album_cate)): $i = 0; $__LIST__ = array_slice($album_cate,0,10,true);if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$val): $mod = ($i % 2 );++$i;?><li><a href="<?php echo u('album/index',array('cid'=>$val['id']));?>" <?php if($val["id"] == $cid): ?>class="on"<?php endif; ?>><?php echo ($val["title"]); ?></a></li><?php endforeach; endif; else: echo "" ;endif; ?>
			 <?php if(!is_null($uid)): ?><li><a href="<?php echo uc('uc/index');?>" target="_blank">我的专辑</a></li><?php endif; ?>
		</ul>
	</div>
    <div class="album_index infinite_scroll clearfix" style="min-height:200px;">
        <ul>
	<?php if(is_array($album_list)): $key = 0; $__LIST__ = $album_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$val): $mod = ($key % 2 );++$key; if($val["items_num"] != 0): ?><li class="album_list" style="<?php if(($key+1)%4 == 1): ?>margin-right:0px;<?php endif; ?>">
					<div class="clearfix title">
						<a class="left" href="<?php echo u('album/details',array('id'=>$val['id'],'uid'=>$val['uid']));?>"><?php echo ($val["title"]); ?></a>
					</div>
					<div class="groupbg">
						<ul>
			            	<?php if(is_array($val['items'])): $k = 0; $__LIST__ = $val['items'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$ival): $mod = ($k % 2 );++$k;?><li <?php if($k == 1): ?>class="first"<?php endif; ?> <?php if(($k == 2) or ($k == 3) or ($k == 6)): ?>style="margin-right: 0;"<?php endif; ?>>
									<a href="<?php echo u('album/details',array('id'=>$val['id'],'uid'=>$val['uid']));?>" title="<?php echo ($val["title"]); ?>" target="_blank">
											<img <?php if($k == 1): ?>style="height:130px; max-width:130px;"<?php endif; ?> src="<?php echo ($ival); ?>" alt="<?php echo ($val["title"]); ?>" />
									</a>
								</li><?php endforeach; endif; else: echo "" ;endif; ?>
						</ul>
					</div>	
					<div class="group_desc mt10">
						<span class="follow_red_btn tc cursor fr">
							<a class="white" target="_blank" href="<?php echo u('album/details',array('id'=>$val['id'],'uid'=>$val['uid']));?>">去看看</a>
						</span>
						<span class="share_nums gray"><?php echo ($val["items_num"]); ?>个宝贝</span>
					</div>		
