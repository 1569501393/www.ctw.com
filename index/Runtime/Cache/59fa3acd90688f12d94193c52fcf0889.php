...
<div class="content">
	...
	<div class="art_left fl">    
        	<ul>
        		 <?php if(is_array($article_list)): $i = 0; $__LIST__ = $article_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$val): $mod = ($i % 2 );++$i;?><li>
                        <span><?php echo (msubstr($val["add_time"],0, 10, "utf-8", false)); ?></span>
                        <?php if(($val['url']) != ""): ?><a href="<?php echo ($val["url"]); ?>"><?php echo ($val["title"]); ?></a>
                        <?php else: ?>
                        <a href="<?php echo u('article/index',array('id'=>$val['id']));?>"><?php echo ($val["title"]); ?></a><?php endif; ?>
                    </li><?php endforeach; endif; else: echo "" ;endif; ?>	
        	</ul>
			<div class="page_b" style="padding-right:15px;padding-top:20px;"><?php echo ($page); ?></div>
    </div>
	  ...
	  <?php if(is_array($article_cate_list['parent'])): $i = 0; $__LIST__ = $article_cate_list['parent'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$val): $mod = ($i % 2 );++$i;?><div class="shoptopshow">	   		
				...
				<ul>	
					<?php if(is_array($article_cate_list['sub'][$val['id']])): $i = 0; $__LIST__ = $article_cate_list['sub'][$val['id']];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$sval): $mod = ($i % 2 );++$i;?><li>	
							<a href="<?php if($sval['url'] != ''): echo ($sval["url"]); else: echo u('articlelist/index',array('cid'=>$sval['id'])); endif; ?>"><?php echo ($sval["name"]); ?></a>							
