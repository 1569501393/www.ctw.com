...
<link rel="stylesheet" type="text/css" href="__TMPL__public/css/item.css" />
...
<div class="content clearfix">
    <div class="item_head mt15">
        <h2 class="red">
			<?php if(isset($pcate_info)): echo ($pcate_info["name"]); ?>			
			<?php else: ?>
				热门<?php endif; ?>			
			<?php if(isset($cate_info)): ?><span class="tag_title gray">&nbsp;&nbsp;<?php echo ($cate_info["name"]); ?></span><?php endif; ?>
		</h2>
		<!-- Baidu Button BEGIN -->
<div id="bdshare" class="bdshare_t bds_tools get-codes-bdshare" style="float:right;"> 
	<a class="bds_qzone"></a> 
    <a class="bds_tsina" pic="logo"></a> 
    <a class="bds_tqq"></a> 
...
<script type="text/javascript">
	 ...
	 */
	var bds_config = {
		'bdPic':def.root+'<?php echo ($site_logo); ?>'//'请参考自定义分享出去的图片'
	}		
        ...
        <div class="box_shadow masonry_brick mt15" style="position: absolute; top: 0px; left: 0px;">
            <div class="catalog clearfix">
            	<div class="catalog_item">
                    <h3 class="no">最新推荐</h3>
                    <div class="clearfix">
                        <?php if(is_array($recommend_cate)): $i = 0; $__LIST__ = $recommend_cate;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$sval): $mod = ($i % 2 );++$i;?><a href="<?php echo u('cate/index',array('cid'=>$sval['id']));?>" style="color:<?php echo ($sval["color"]); ?>;"
                           <?php if($request["cid"] == $sval['id']): ?>class="on"<?php endif; ?>><?php echo ($sval['name']); ?></a><?php endforeach; endif; else: echo "" ;endif; ?>
                    </div>
                </div>            	
                <?php if(is_array($cate_list)): $i = 0; $__LIST__ = $cate_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$val): $mod = ($i % 2 );++$i;?><div class="catalog_item">
                    <h3><?php echo ($val["name"]); ?></h3>
                    <div class="clearfix">
                        <ul>
                        <?php if(is_array($val['items'])): $i = 0; $__LIST__ = $val['items'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$sval): $mod = ($i % 2 );++$i; if($sval['recommend'] == 0): ?><li class="clearfixli"><a href="<?php echo u('cate/index',array('cid'=>$sval['id']));?>" style="color:<?php echo ($sval["color"]); ?>;"
	                           <?php if($request["cid"] == $sval['id']): ?>class="on"<?php endif; ?>><?php echo ($sval['name']); ?></a></li><?php endif; ?>
                            <!--a href="<?php echo u('cate/index',array('cid'=>$sval['id']));?>" style="color:<?php echo ($sval["color"]); ?>;"
                           <?php if($request["cid"] == $sval['id']): ?>class="on"<?php endif; ?>><?php echo ($sval['name']); ?></a--><?php endforeach; endif; else: echo "" ;endif; ?>
                        </ul>
                    </div>
...
<div class="user clearfix">
    ...
    <?php if($show_sp == 1): ?><div id="more" class="center">
            <a href="<?php echo u('cate/index',array('sp'=>2,'cid'=>$cid,'p'=>$p));?>" hidefocus="true"></a>
        </div><?php endif; ?> 
