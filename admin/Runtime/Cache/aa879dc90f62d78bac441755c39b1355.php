...
</script><title><?php echo (L("website_manage")); ?></title></head><body><div id="ajax_loading">提交请求中，请稍候...</div><?php if($show_header != false): if(($sub_menu != '') OR ($big_menu != '')): ?><div class="subnav"><div class="content-menu ib-a blue line-x"><?php if(!empty($big_menu)): ?><a class="add fb" href="<?php echo ($big_menu["0"]); ?>"><em><?php echo ($big_menu["1"]); ?></em></a>　<?php endif; ?></div></div><?php endif; endif; ?><script type="text/javascript" src="__ROOT__/statics/js/jquery/plugins/jquery.imagePreview.js"></script><div class="pad-lr-10"><form name="searchform" action="" method="get" ><table width="100%" cellspacing="0" class="search-form"><tbody><tr><td><div class="explain-col">                &nbsp;订单号/商品名称 :
                <input name="keyword" type="text" class="input-text" size="25" value="<?php echo ($keyword); ?>" /><input type="hidden" name="m" value="exchange_order" /><input type="submit" name="search" class="button" value="搜索" /></div></td></tr></tbody></table></form><form id="myform" name="myform" action="<?php echo u('exchange_order/delete');?>" method="post" onsubmit="return check();"><div class="table-list"><table width="100%" cellspacing="0"><thead><tr><th width=50>ID</th><th width=30><input type="checkbox" value="" id="check_box" onclick="selectall('id[]');"></th><th width=100><?php echo L('sn');?></th><th width=400><?php echo L('data_name');?></th><th width=60><?php echo L('data_num');?></th><th width=120><?php echo L('integral');?></th><th width=120><?php echo L('user_name');?></th><th width=120><?php echo L('create_time');?></th><th width=120><?php echo L('goods_status');?></th><th width=120>操作</th></tr></thead><tbody><?php if(is_array($ex_order_list)): $k = 0; $__LIST__ = $ex_order_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$val): $mod = ($k % 2 );++$k;?><tr><td align="center"><?php echo ($val["id"]); ?></td><td align="center"><input type="checkbox" value="<?php echo ($val["id"]); ?>" name="id[]" </td><td align="left"><?php echo ($val["sn"]); ?></td><td align="center"><?php echo ($val["data_name"]); ?></td><td align="center"><?php echo ($val["data_num"]); ?></td><td align="center"><?php echo ($val["order_score"]); ?></td><td align="center"><?php echo ($val["user_name"]); ?></td><td align="center"><?php echo (date("Y-m-d",$val["create_time"])); ?></td><td align="center"><?php if($val["goods_status"] == '0' ): ?>未发货<?php endif; if($val["goods_status"] == '1' ): ?>已发货<?php endif; ?></td><td align="center"><a href="<?php echo u('exchange_order/edit',array('id'=>$val['id']));?>">编辑</a></td></tr><?php endforeach; endif; else: echo "" ;endif; ?></tbody></table><div class="btn"><label for="check_box" style="float:left;"><?php echo (L("select_all")); ?>/<?php echo (L("cancel")); ?></label><input type="submit" class="button" name="dosubmit" value="<?php echo (L("delete")); ?>" onclick="return confirm('<?php echo (L("sure_delete")); ?>')" style="float:left;margin-left:10px;"/><div id="pages"><?php echo ($page); ?></div></div></div></form></div><script language="javascript">$(function(){
	$(".preview").preview();
});
