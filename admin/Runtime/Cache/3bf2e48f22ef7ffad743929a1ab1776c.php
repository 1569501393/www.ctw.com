...
$(function($){
	$("#ajax_loading").ajaxStart(function(){
		...
		$("#board_id").change();
...
</script><div class="pad_10"><form action="<?php echo u('ad/add');?>" method="post" name="myform" id="myform" enctype="multipart/form-data"><table width="100%" cellpadding="2" cellspacing="1" class="table_form"><tr><th width="80"><?php echo L('name');?>:</th><td><input type="text" name="name" id="name" class="input-text" size="40"></td></tr><tr><th><?php echo L('url');?> :</th><td><input type="text" name="url" id="url" class="input-text" size="40"></td></tr><tr><th><?php echo L('board_id');?> :</th><td><select name="board_id" id="board_id"><?php if(is_array($adboards)): $i = 0; $__LIST__ = $adboards;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$val): $mod = ($i % 2 );++$i;?><option value="<?php echo ($val["id"]); ?>" allowtype="<?php echo ($val["allow_type"]); ?>"><?php echo ($val["name"]); ?>（<?php echo ($val["width"]); ?>*<?php echo ($val["height"]); ?>）</option><?php endforeach; endif; else: echo "" ;endif; ?></select></td></tr><tr><th><?php echo L('type');?> :</th><td><select name="type" id="type"><?php if(is_array($ad_type_arr)): $i = 0; $__LIST__ = $ad_type_arr;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$val): $mod = ($i % 2 );++$i;?><option value="<?php echo ($key); ?>"><?php echo ($val); ?></option><?php endforeach; endif; else: echo "" ;endif; ?></select></td></tr><tr id="ad_text" class="bill_media"><th><?php echo L('text');?> :</th><td><textarea rows="4" cols="50" name="text" id="text"></textarea></td></tr><tr id="ad_code" class="bill_media"><th><?php echo L('code');?> :</th><td><textarea rows="7" cols="65" name="code" id="code"></textarea></td></tr><tr id="ad_image" class="bill_media"><th><?php echo L('ad_image');?> :</th><td><input name="image" id="image" type="file" /></td></tr><tr id="ad_flash" class="bill_media"><th><?php echo L('ad_flash');?> :</th><td><input name="flash" id="flash" type="file" /></td></tr><tr><th><?php echo L('start_time');?> :</th><td><link rel="stylesheet" type="text/css" href="__ROOT__/statics/js/calendar/calendar-blue.css"/><script type="text/javascript" src="__ROOT__/statics/js/calendar/calendar.js"></script><input class="date input-text" type="text" name="start_time" id="start_time" size="18" value="" /><script language="javascript" type="text/javascript">	                    Calendar.setup({
	     ...
	     </script></td></tr><tr><th><?php echo L('status');?> :</th><td><input type="radio" checked="checked" value="1" name="status"> 开启 &nbsp;&nbsp;
