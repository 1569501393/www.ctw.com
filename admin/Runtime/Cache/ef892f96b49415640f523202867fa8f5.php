...
</script><form id="myform" name="myform" action="<?php echo u(MODULE_NAME.'/delete');?>" method="post" onsubmit="return check();"><div class="table-list"><table width="100%" cellspacing="0"><thead><tr><th width="50">ID</th><th width=15><input type="checkbox" value="" id="check_box" onclick="selectall('id[]');"></th><th><?php echo L('title');?></th><th><?php echo L('album_num');?></th><th width=120><?php echo L('last_time');?></th><th width="30"><?php echo L('sort_order');?></th><th width=30><?php echo L('status');?></th><th width=30><?php echo L('edit');?></th></tr></thead><tbody><?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$val): $mod = ($i % 2 );++$i;?><tr><td align="center"><?php echo ($val["id"]); ?></td><td align="center"><input type="checkbox" value="<?php echo ($val["id"]); ?>" name="id[]"></td><td align="center"><em class="blue"><?php echo ($val["title"]); ?></em></td><td  align="center"><?php echo ($val["album_num"]); ?></td><td align="center"><?php echo date("Y-n-j   H:i:s",$val["add_time"]);?></td><td><input type="text" class="input-text-c input-text" id="sort_<?php echo ($val["id"]); ?>" onblur="sort(<?php echo ($val["id"]); ?>,'sort_order',this.value)" value="<?php echo ($val["sort_order"]); ?>" size="4" name="listorders[<?php echo ($val["id"]); ?>]"   onkeyup="this.value=this.value.replace(/D/g,'')" onafterpaste="this.value=this.value.replace(/D/g,'')"><!--id="listorders[<?php echo ($val["id"]); ?>]"--></td><td align="center" onclick="status(<?php echo ($val["id"]); ?>,'status')" id="status_<?php echo ($val["id"]); ?>"><img src="__ROOT__/statics/images/status_<?php echo ($val["status"]); ?>.gif" /></td><td align="center" onclick="status(<?php echo ($val["id"]); ?>,'status')" id="status_<?php echo ($val["id"]); ?>"><a href="<?php echo u(MODULE_NAME.'/edit',array('id'=>$val['id']));?>" target="_blank">编辑</a></td><?php endforeach; endif; else: echo "" ;endif; ?></tbody></table><div class="btn"><label for="check_box" style="float:left;">全选/取消</label><input type="submit" class="button" name="dosubmit" value="<?php echo (L("delete")); ?>" onclick="return confirm('<?php echo (L("sure_delete")); ?>')" style="float:left;margin:0 10px 0 10px;"/><div id="pages"><?php echo ($page); ?></div></div></div></form></div><script>//排序方法
function sort(id,type,num){
    
    $.get("<?php echo u('album_cate/sort');?>", { id: id, type: type,num:num }, function(jsondata){
